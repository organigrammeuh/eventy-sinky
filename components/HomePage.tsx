...
async function fetchSpeakers() {
    try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL ?? "http://localhost:3000"}/api/speakers`, {
            cache: "no-store",
        });
        if (!res.ok) return [];
        return await res.json();
        ...
        <main>
            <section className="relative min-h-screen flex flex-col items-center justify-center text-center px-6 pt-28 pb-20 overflow-hidden">
                ...
                <div className="flex gap-3 flex-wrap justify-center">
                    <Link
                        href="/events"
                        className="px-7 py-3 rounded-full bg-white text-[#3C3489] font-semibold text-sm no-underline transition-opacity hover:opacity-90"
                    >
                        View all events
                    </Link>
                    <Link
            ...
            <section className="py-20 px-6 max-w-[1100px] mx-auto">
                <div className="flex items-baseline justify-between mb-8">
                    ...
                    <Link href="/events" className="text-sm text-primary font-medium no-underline">
                        View all →
                    </Link>
                </div>
            ...
            <section className="py-20 px-6 bg-muted">
                <div className="max-w-[1100px] mx-auto">
                    <div className="flex items-baseline justify-between mb-8">
                        ...
                        <Link href="/sessions" className="text-sm text-primary font-medium no-underline">
                            View all →
                        </Link>
                    </div>
                    ...
                    <div className="grid grid-cols-[repeat(auto-fill,minmax(260px,1fr))] gap-4">
                        {events
                            .flatMap((e: any) =>
                                    (e.sessions ?? []).slice(0, 2).map((s: Session) => {
                                        const now = new Date();
                                        const live =
            ...
            <section className="py-20 px-6 max-w-[1100px] mx-auto">
                <div className="flex items-baseline justify-between mb-8">
                    ...
                    <Link href="/speakers" className="text-sm text-primary font-medium no-underline">
                        View all →
                    </Link>
                </div>
                            ...
                            <div className="card-hover bg-card border border-card-border rounded-xl px-6 py-5 flex items-center gap-3 min-w-[200px]">
                                {speaker.profilePicture ? (
                                    <img
                                        ...
                                        className="w-20 h-20 rounded-full object-cover border-2 border-primary-muted flex-shrink-0"
                                    />
                                ) : (
                                    <div className="w-12 h-12 rounded-full bg-primary-muted text-primary flex items-center justify-center font-bold text-sm flex-shrink-0">
